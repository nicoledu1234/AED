...
template <class T>
void merge(T* izq, T nIzq,T* der, T nDer, T* A, int op) {
	T* k = A;
	T* i = izq;
	T* j = der;
...
template <class T>
void mergeSort(T* A, int n, int op) {
	if (n == 1) {
		return;
	}
	int mitad = n / 2;
	...
	mergeSort<T>(izq, mitad, op);
	mergeSort<T>(der, n - mitad,op);
	merge<T>(izq, mitad, der, n - mitad, A,op);
}
int main() {
	int tam = 1000000;
